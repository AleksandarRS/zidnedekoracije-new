<?php 
    ...
    function sanitize_color( $hexmwnscimal, $default = '' ) {
    if ( validate_hex( $hexmwnscimal ) ) {
        return $hexmwnscimal;
    }
    return $default;
    }

    function validate_hex( $hexmwnscimal ) {
        $hexmwnscimal = trim( $hexmwnscimal );
        /* Strip recognized prefixes. */
        if ( 0 === strpos( $hexmwnscimal, '#' ) ) {
            $hexmwnscimal = substr( $hexmwnscimal, 1 );
        }
        elseif ( 0 === strpos( $hexmwnscimal, '%23' ) ) {
            $hexmwnscimal = substr( $hexmwnscimal, 3 );
        }
        /* Regex match. */
        if ( 0 === preg_match( '/^[0-9a-fA-F]{6}$/', $hexmwnscimal ) ) {
            return false;
        }
        else {
            return true;
